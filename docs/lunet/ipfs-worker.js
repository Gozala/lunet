...
const request = async (message /*:Data.RequestMessage*/, target) => {
  try {
...
const match = (message /*:Data.WorkerIncomingMessage*/, target) => {
  switch (message.type) {
    case "ping": {
      return ping(message, target)
    }
    case "request": {
...
const ping = (message, target) => {
  target.postMessage({ type: "pong", time: Date.now() })
}
...
const respond = async request => {
  ...
  switch (pathname) {
    ...
    case "/api/v0/object/patch/rm-link": {
      ...
      const rootCID = new ipfs.CID(root)
      const cid = await ipfs.daemon.object.patch.rmLink(rootCID, { name: link })
      const body = await ipfs.object$get(cid, base, false)
      return encodeDaemonResponse({
        url: request.url,
        body
    ...
    case "/api/v0/pin/ls": {
      ...
      return encodeDaemonResponse({ url: request.url, body: { Keys: pinset } })
    }
    case "/api/v0/dns": {
    ...
    default: {
      if (pathname.startsWith("/ipfs/") || pathname.startsWith("/ipns/")) {
        return encodeResponse(await ipfs.gateway.fetch(pathname), request.url)
      } else {
...
class IPFS {
  ...
  */
  static new() {
    return new this(new self.Ipfs())
  }
  constructor(daemon) {
    this.daemon = daemon
    this.ready = onready(daemon)
    this.gateway = new Gateway(daemon)
    this.Buffer = daemon.types.Buffer
    this.multiaddr = daemon.types.multiaddr
    this.CID = daemon.types.CID
    this.dagPB = daemon.types.dagPB
  }
  // Copied from
  // https://github.com/ipfs/js-ipfs/blob/bbe561b7b1bf068eb703082c30cefa0f3e659d4d/src/utils/cid.js#L20-L35
  cidToString(
    input,
    ...
    if (!this.CID.isCID(cid)) {
      cid = new this.CID(cid)
    }
  ...
  async object$get(cid, base, upgrade, dataEncoding) {
    ...
    const { data, size, links } = node.toJSON()
    const content = ipfs.Buffer.isBuffer(data)
...
class Gateway {
  ...
  */
  ...
  fetch(path) /*:Promise<Response>*/ {
    return self.IpfsHttpResponse.getResponse(this.daemon, path)
  }
}
